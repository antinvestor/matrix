...
func AddPublicRoutes(
	...
	if err != nil {
		logrus.With(slog.Any("error", err)).Panicf("failed to connect to sync db")
	}
	...
	if err = notifier.Load(context.Background(), syncDB); err != nil {
		logrus.With(slog.Any("error", err)).Panicf("failed to load notifier ")
	}
	...
	if dendriteCfg.SyncAPI.Fulltext.Enabled {
		...
		if err != nil {
			logrus.With(slog.Any("error", err)).Panicf("failed to create full text")
		}
	}
	...
	if err = presenceConsumer.Start(); err != nil {
		logrus.With(slog.Any("error", err)).Panicf("failed to start presence consumer")
	}
	...
	if err = keyChangeConsumer.Start(); err != nil {
		logrus.With(slog.Any("error", err)).Panicf("failed to start key change consumer")
	}
	...
	if err = roomConsumer.Start(); err != nil {
		logrus.With(slog.Any("error", err)).Panicf("failed to start room server consumer")
	}
	...
	if err = clientConsumer.Start(); err != nil {
		logrus.With(slog.Any("error", err)).Panicf("failed to start client data consumer")
	}
	...
	if err = notificationConsumer.Start(); err != nil {
		logrus.With(slog.Any("error", err)).Panicf("failed to start notification data consumer")
	}
	...
	if err = typingConsumer.Start(); err != nil {
		logrus.With(slog.Any("error", err)).Panicf("failed to start typing consumer")
	}
	...
	if err = sendToDeviceConsumer.Start(); err != nil {
		logrus.With(slog.Any("error", err)).Panicf("failed to start send-to-device consumer")
	}
	...
	if err = receiptConsumer.Start(); err != nil {
		logrus.With(slog.Any("error", err)).Panicf("failed to start receipts consumer")
	}
