	...
	for i := 0; i < len(in); i++ {
		...
		if device != nil && in[i].TransactionID != nil {
			...
			if err != nil {
				...
				}).With(slog.Any("error", err)).Warn("Failed to add transaction ID to event")
				continue
			}
...
func (d *Database) RedactEvent(ctx context.Context, redactedEventID string, redactedBecause *rstypes.HeaderedEvent, querier api.QuerySenderIDAPI) error {
	...
	if len(redactedEvents) == 0 {
		logrus.With("event_id", redactedEventID).With("redaction_event", redactedBecause.EventID()).Warn("missing redacted event for redaction")
		return nil
	}
...
func (d *Database) CleanSendToDeviceUpdates(
	...
	if err = d.Writer.Do(d.DB, nil, func(txn *sql.Tx) error {
		...
		logrus.With(slog.Any("error", err)).Error("Failed to clean up old send-to-device messages for user %q device %q", userID, deviceID)
		return err
	}
	return nil
}
...
func (d *Database) UpdateRelations(ctx context.Context, event *rstypes.HeaderedEvent) error {
	...
	if err := json.Unmarshal(event.Content(), &content); err != nil {
		logrus.With(slog.Any("error", err)).Error("unable to unmarshal relation content")
		return nil
	}
	switch {
