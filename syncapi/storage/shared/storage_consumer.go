...
func (d *Database) AddPeek(ctx context.Context, roomID, userID, deviceID string) (sp types.StreamPosition, err error) {
	return d.Peeks.InsertPeek(ctx, roomID, userID, deviceID)
}
...
func (d *Database) WriteEvent(
	...
	if err != nil {
		return 0, fmt.Errorf("d.OutputEvents.InsertEvent: %w", err)
	}
	pduPosition = pos
	var topoPosition types.StreamPosition
	if topoPosition, err = d.Topology.InsertEventInTopology(ctx, ev, pos); err != nil {
		return 0, fmt.Errorf("d.Topology.InsertEventInTopology: %w", err)
	}

	if err = d.handleBackwardExtremities(ctx, ev); err != nil {
		return 0, fmt.Errorf("d.handleBackwardExtremities: %w", err)
	}

	if len(addStateEvents) == 0 && len(removeStateEventIDs) == 0 {
		// Nothing to do, the event may have just been a message event.
		return pduPosition, nil
	}
	for i := range addStateEvents {
		addStateEvents[i].Visibility = historyVisibility
	}
	return pduPosition, d.updateRoomState(ctx, removeStateEventIDs, addStateEvents, pduPosition, topoPosition)
}
...
func (d *Database) StoreNewSendForDeviceMessage(
	...
	newPos, err = d.SendToDevice.InsertSendToDeviceMessage(ctx, userID, deviceID, string(j))
	return
}
...
func (d *Database) StoreReceipt(ctx context.Context, roomId, receiptType, userId, eventId string, timestamp spec.Timestamp) (pos types.StreamPosition, err error) {
	return d.Receipts.UpsertReceipt(ctx, roomId, receiptType, userId, eventId, timestamp)
}
...
func (d *Database) UpsertRoomUnreadNotificationCounts(ctx context.Context, userID, roomID string, notificationCount, highlightCount int) (pos types.StreamPosition, err error) {
	return d.NotificationData.UpsertRoomUnreadCounts(ctx, userID, roomID, notificationCount, highlightCount)
}
...
func getMembershipFromEvent(ctx context.Context, ev gomatrixserverlib.PDU, userID string, rsAPI api.QuerySenderIDAPI) (string, string) {
