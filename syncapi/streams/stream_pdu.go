...
func (p *PDUStreamProvider) Setup(
	...
	id, err := p.DB.MaxStreamPositionForPDUs(ctx)
	if err != nil {
		panic(err)
	}
	p.latest = id
...
func (p *PDUStreamProvider) CompleteSync(
	...
	joinedRoomIDs, err := p.DB.RoomIDsWithMembership(ctx, req.Device.UserID, spec.Join)
	if err != nil {
	...
	if err = p.addIgnoredUsersToFilter(ctx, p.DB, req, &eventFilter); err != nil {
	...
	recentEvents, err := p.DB.RecentEvents(ctx, joinedRoomIDs, r, &eventFilter, true, true)
	if err != nil {
		return from
	}
	...
	for _, roomID := range joinedRoomIDs {
		...
		jr, jerr := p.getJoinResponseForCompleteSync(
			ctx, p.DB, roomID, &stateFilter, req.WantFullState, req.Device, false,
	...
	if len(peeks) > 0 {
		...
		recentEvents, err = p.DB.RecentEvents(ctx, peekRooms, r, &eventFilter, true, true)
		if err != nil {
			return from
		}
		...
		for _, roomID := range peekRooms {
			...
			jr, err = p.getJoinResponseForCompleteSync(
				ctx, p.DB, roomID, &stateFilter, req.WantFullState, req.Device, true,
				events.Events, events.Limited, eventFormat,
			)
			if err != nil {
...
func (p *PDUStreamProvider) IncrementalSync(
	...
	if req.WantFullState {
		if stateDeltas, syncJoinedRooms, err = p.DB.GetStateDeltasForFullStateSync(ctx, req.Device, r, req.Device.UserID, &stateFilter, p.rsAPI); err != nil {
			log.WithError(err).Error("p.Cm.GetStateDeltasForFullStateSync failed")
			return from
		}
	} else {
		if stateDeltas, syncJoinedRooms, err = p.DB.GetStateDeltas(ctx, req.Device, r, req.Device.UserID, &stateFilter, p.rsAPI); err != nil {
	...
	dbEvents, err := p.getRecentEvents(ctx, stateDeltas, r, eventFilter, p.DB)
	if err != nil {
	...
	for _, delta := range stateDeltas {
		...
		if pos, err = p.addRoomDeltaToResponse(ctx, p.DB, req.Device, newRange, delta, &eventFilter, &stateFilter, req, dbEvents); err != nil {
...
func (p *PDUStreamProvider) addRoomDeltaToResponse(
	...
	case spec.Join:
		...
		jr.Timeline.Events = synctypes.ToClientEvents(ctx, gomatrixserverlib.ToPDUs(events), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
		...
		jr.State.Events = synctypes.ToClientEvents(ctx, gomatrixserverlib.ToPDUs(delta.StateEvents), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, roomID, senderID)
		})
	...
	case spec.Peek:
		...
		jr.Timeline.Events = synctypes.ToClientEvents(ctx, gomatrixserverlib.ToPDUs(recentEvents), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, roomID, senderID)
		})
	...
	case spec.Ban:
		...
		lr.Timeline.Events = synctypes.ToClientEvents(ctx, gomatrixserverlib.ToPDUs(events), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, roomID, senderID)
		})
		// If we are limited by the filter AND the history visibility filter
		// didn't "remove" events, return that the response is limited.
		lr.Timeline.Limited = limited && len(events) == len(recentEvents)
		lr.State.Events = synctypes.ToClientEvents(ctx, gomatrixserverlib.ToPDUs(delta.StateEvents), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, roomID, senderID)
		})
...
func (p *PDUStreamProvider) getJoinResponseForCompleteSync(
	...
	jr.State.Events = synctypes.ToClientEvents(ctx, gomatrixserverlib.ToPDUs(stateEvents), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
		return p.rsAPI.QueryUserIDForSender(ctx, roomID, senderID)
	})
	return jr, nil
}
