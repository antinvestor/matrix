...
func createTransactionWithEDU(ctx context.Context, _ *frame.Service, cfg *config.Matrix, t *testing.T, edus []gomatrixserverlib.EDU) (*config.Matrix, TxnReq, nats.JetStreamContext) {
...
func TestProcessTransactionRequestEDUTyping(t *testing.T) {
	...
	cfg, txn, js := createTransactionWithEDU(ctx, svc, cfg, t, edus)
...
func TestProcessTransactionRequestEDUUnhandled(t *testing.T) {
	...
	_, txn, _ := createTransactionWithEDU(ctx, svc, cfg, t, []gomatrixserverlib.EDU{edu})
