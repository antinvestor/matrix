...
func DirectoryRoom(
	...
	if err = rsAPI.GetRoomIDForAlias(req.Context(), queryReq, queryRes); err != nil {
		util.GetLogger(req.Context()).With(slog.Any("error", err)).Error("rsAPI.GetRoomIDForAlias failed")
		return util.JSONResponse{
	...
	if res.RoomID == "" {
		...
		if err = fedSenderAPI.QueryJoinedHostServerNamesInRoom(req.Context(), &joinedHostsReq, &joinedHostsRes); err != nil {
			util.GetLogger(req.Context()).With(slog.Any("error", err)).Error("fedSenderAPI.QueryJoinedHostServerNamesInRoom failed")
			return util.JSONResponse{
				Code: http.StatusInternalServerError,
				JSON: spec.InternalServerError{},
			}
...
func SetLocalAlias(
	...
	if err != nil {
		util.GetLogger(req.Context()).With(slog.Any("error", err)).Error("QuerySenderIDForUser failed")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.Unknown("internal server error"),
		}
	} else if senderID == nil {
		util.GetLogger(req.Context()).With("roomID", *roomID).With("userID", *userID).Error("Sender ID not found")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.Unknown("internal server error"),
		}
	...
	if err != nil {
		util.GetLogger(req.Context()).With(slog.Any("error", err)).Error("aliasAPI.SetRoomAlias failed")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
...
func RemoveLocalAlias(
	...
	if err != nil {
		util.GetLogger(req.Context()).With(slog.Any("error", err)).Error("roomserverAPI.QueryMembershipForUser failed")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.Unknown("internal server error"),
		}
	...
	if err != nil {
		util.GetLogger(req.Context()).With(slog.Any("error", err)).Error("aliasAPI.RemoveRoomAlias failed")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.Unknown("internal server error"),
		}
...
func GetVisibility(
	...
	if err != nil {
		util.GetLogger(req.Context()).With(slog.Any("error", err)).Error("QueryPublishedRooms failed")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
...
func SetVisibility(
	...
	if err != nil {
		util.GetLogger(req.Context()).With(slog.Any("error", err)).Error("roomID is invalid")
		return util.JSONResponse{
			Code: http.StatusBadRequest,
	...
	if err != nil || len(queryEventsRes.StateEvents) == 0 {
		util.GetLogger(req.Context()).With(slog.Any("error", err)).Error("could not query events from room")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
	...
	if err = rsAPI.PerformPublish(req.Context(), &roomserverAPI.PerformPublishRequest{
		...
		util.GetLogger(req.Context()).With(slog.Any("error", err)).Error("failed to publish room")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
