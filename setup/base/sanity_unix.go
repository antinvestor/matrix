...
func PlatformSanityChecks() {
	...
	warn := func(rLimit *syscall.Rlimit) {
		logrus.Warn("IMPORTANT: Process file descriptor limit is currently %d, it is recommended to raise the limit for Dendrite to at least 65535 to avoid issues", rLimit.Cur)
	}
	...
	if err := syscall.Getrlimit(syscall.RLIMIT_NOFILE, &rLimit); err == nil && rLimit.Cur < 65535 {
		...
		if err = syscall.Setrlimit(syscall.RLIMIT_NOFILE, &rLimit); err != nil {
			...
			logrus.With(slog.Any("error", err)).Warn("IMPORTANT: Failed to raise the file descriptor limit")
			warn(&rLimit)
