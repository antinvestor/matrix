...
func NewServerACLs(db ServerACLDatabase) *ServerACLs {
	...
	if err != nil {
		logrus.With(slog.Any("error", err)).Fatalf("Failed to get known rooms")
	}
	...
	if err != nil {
		logrus.With(slog.Any("error", err)).Error("Failed to get server ACLs for all rooms: %q", err)
	}
...
func (s *ServerACLs) OnServerACLUpdate(strippedEvent tables.StrippedEvent) {
	...
	if err := json.Unmarshal([]byte(strippedEvent.ContentValue), &acls.ServerACL); err != nil {
		logrus.With(slog.Any("error", err)).Error("Failed to unmarshal state content for server ACLs")
		return
	}
	...
	for _, orig := range acls.Allowed {
		if expr, err := s.cachedCompileACLRegex(orig); err != nil {
			logrus.With(slog.Any("error", err)).Error("Failed to compile allowed regex")
		} else {
	...
	for _, orig := range acls.Denied {
		if expr, err := s.cachedCompileACLRegex(orig); err != nil {
			logrus.With(slog.Any("error", err)).Error("Failed to compile denied regex")
		} else {
