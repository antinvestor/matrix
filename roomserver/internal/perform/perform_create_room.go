...
func (c *Creator) PerformCreateRoom(ctx context.Context, userID spec.UserID, roomID spec.RoomID, createRequest *api.PerformCreateRoomRequest) (string, *util.JSONResponse) {
	...
	if createRequest.CreateEvent != nil {
		...
		if err != nil {
			util.Log(ctx).WithError(err).Error("gomatrixserverlib.NewEventFromTrustedJSON failed to verify create event")
			return "", &util.JSONResponse{
				Code: http.StatusInternalServerError,
				JSON: spec.InternalServerError{},
			}
		...
		if err = authEvents.AddEvent(createEvent); err != nil {
			util.Log(ctx).WithError(err).Error("gomatrixserverlib.AuthEvents.AddEvent failed")
			return "", &util.JSONResponse{
				Code: http.StatusInternalServerError,
				JSON: spec.InternalServerError{},
			}
		...
		if contentErr != nil {
			util.Log(ctx).WithError(contentErr).Error("GenerateCreateContent failed")
			return "", &util.JSONResponse{
				Code: http.StatusBadRequest,
		...
		if jsonErr != nil {
			util.Log(ctx).WithField("err", jsonErr).Error("Failed to make the create event")
			return "", jsonErr
		}
		if err = authEvents.AddEvent(createEvent); err != nil {
