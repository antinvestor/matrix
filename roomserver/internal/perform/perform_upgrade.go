...
func (r *Upgrader) performRoomUpgrade(
	...
	if err != nil {
		util.Log(ctx).WithError(err).Error("GenerateCreateContent failed")
		return "", fmt.Errorf("failed to GenerateCreateContent")
	}
	authEvents, _ := gomatrixserverlib.NewAuthEvents(nil)
	identity, err := r.Cfg.Global.SigningIdentityFor(userID.Domain())
	if err != nil {
		util.Log(ctx).WithError(err).Error("Failed to get signing identity")
	...
	if jsonErr != nil {
		util.Log(ctx).Error("Failed to make the create event")
