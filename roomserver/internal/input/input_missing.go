...
func (t *missingStateReq) processEventWithMissingState(
	...
	if isGapFilled {
		t.log.WithField("event_id", e.EventID()).
			WithField("room_id", e.RoomID().String()).
			WithField("txn_prev_events", e.PrevEventIDs()).
			Info("Gap filled by /get_missing_events, injecting %d new events", len(newEvents))
	...
	if isGapFilled && prevStatesKnown {
		t.log.WithField("event_id", e.EventID()).
			WithField("room_id", e.RoomID().String()).
			WithField("txn_prev_events", e.PrevEventIDs()).
			Info("Gap filled and state found for all prev events")
		return nil, nil
	}
...
func (t *missingStateReq) lookupStateAfterEventLocally(ctx context.Context, eventID string) *parsedRespState {
	...
	if err != nil {
		t.log.WithError(err).Warn("failed to get state after %s locally", eventID)
		return nil
	}
	stateEntries, err := roomState.LoadCombinedStateAfterEvents(ctx, stateAtEvents)
	if err != nil {
		t.log.WithError(err).Warn("failed to load combined state after %s locally", eventID)
		return nil
	}
	...
	if err != nil {
		t.log.WithError(err).Warn("failed to load state events locally")
		return nil
	}
	...
	if len(missingAuthEvents) > 0 {
		...
		t.log.WithField("count", len(missingEventList)).Debug("Fetching missing auth events")
...
retryAllowedState:
	if err = checkAllowedByState(backwardsExtremity, resolvedStateEvents, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
		...
		case gomatrixserverlib.MissingAuthEventError:
			...
			t.log.Debug("fetched event %s", missing.AuthEventID)
...
func (t *missingStateReq) getMissingEvents(ctx context.Context, e gomatrixserverlib.PDU, roomVersion gomatrixserverlib.RoomVersion) (newEvents []gomatrixserverlib.PDU, isGapFilled, prevStateKnown bool, err error) {
	...
	logger := t.log.WithField("event_id", e.EventID()).
		WithField("room_id", e.RoomID().String())
	...
	for _, server := range t.servers {
		...
		if m, err = t.federation.LookupMissingEvents(ctx, t.virtualHost, server, e.RoomID().String(), fclient.MissingEvents{
			...
			logger.WithError(err).Warn("%s pushed us an event but %q did not respond to /get_missing_events", t.origin, server)
	...
	logger.WithField("count", len(missingResp.Events)).
		WithField("passed", len(missingEvents)).
		Debug("get_missing_events returned %d events (%d passed signature checks)", len(missingResp.Events), len(missingEvents))
...
func (t *missingStateReq) lookupMissingStateViaStateIDs(ctx context.Context, roomID, eventID string, roomVersion gomatrixserverlib.RoomVersion) (
	*parsedRespState, error) {
	...
	t.log.WithField("event_id", eventID).Info("lookupMissingStateViaStateIDs %s", eventID)
	...
	t.log.WithField("event_id", eventID).
		WithField("missing", missingCount).
		WithField("total_state", len(stateIDs.GetStateEventIDs())).
		WithField("total_auth_events", len(stateIDs.GetAuthEventIDs())).
		Debug("lookupMissingStateViaStateIDs missing %d/%d events", missingCount, len(wantIDs))
	...
	if missingCount > concurrentRequests && missingCount > len(wantIDs)/2 {
		t.log.WithField("missing", missingCount).
	...
	if missingCount > 0 {
		t.log.WithField("missing", missingCount).
			...
			WithField("total_state", len(stateIDs.GetStateEventIDs())).
			WithField("total_auth_events", len(stateIDs.GetAuthEventIDs())).
			WithField("concurrent_requests", concurrentRequests).
		...
		fetch := func(missingEventID string) {
			...
			case nil:
				break
			default:
				t.log.WithField("missing_event_id", missingEventID).
	...
	resp, err := t.createRespStateFromStateIDs(ctx, stateIDs)
	return resp, err
}
...
func (t *missingStateReq) lookupEvent(ctx context.Context, roomVersion gomatrixserverlib.RoomVersion, _, missingEventID string, localFirst bool) (gomatrixserverlib.PDU, error) {
	...
	if localFirst {
		...
		if err != nil {
			t.log.WithError(err).Warn("Failed to query roomserver for missing event %s: %s - falling back to remote", missingEventID)
...
serverLoop:
	for _, serverName := range t.servers {
		...
		if err != nil || len(txn.PDUs) == 0 {
			t.log.WithError(err).
				WithField("missing_event_id", missingEventID).
				Warn("Failed to get missing /event for event ID")
			if errors.Is(err, context.DeadlineExceeded) {
				select {
		...
		default:
			t.log.WithError(err).
				WithField("missing_event_id", missingEventID).
				Warn("Failed to parse event JSON of event returned from /event")
			continue
		}
	}
	if !found {
		t.log.WithField("missing_event_id", missingEventID).
			Warn("Failed to get missing /event for event ID from %d server(s)", len(t.servers))
	...
	if err := gomatrixserverlib.VerifyEventSignatures(ctx, event, t.keys, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
		...
		t.log.WithError(err).
			...
			Warn("Couldn't validate signature of event %q from /event")
