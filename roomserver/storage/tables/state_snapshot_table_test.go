...
func mustCreateStateSnapshotTable(ctx context.Context, svc *frame.Service, t *testing.T) tables.StateSnapshot {
	t.Helper()

	cm := migrateDatabase(ctx, svc, t)

	tab := postgres.NewPostgresStateSnapshotTable(cm)
...
func TestStateSnapshotTable(t *testing.T) {
	test.WithAllDatabases(t, func(t *testing.T, testOpts test.DependancyOption) {
		...
		tab := mustCreateStateSnapshotTable(ctx, svc, t)
