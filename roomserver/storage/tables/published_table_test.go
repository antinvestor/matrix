...
func mustCreatePublishedTable(ctx context.Context, svc *frame.Service, t *testing.T) tables.Published {
	t.Helper()

	cm := migrateDatabase(ctx, svc, t)
	tab := postgres.NewPostgresPublishedTable(cm)
...
func TestPublishedTable(t *testing.T) {
	...
	test.WithAllDatabases(t, func(t *testing.T, testOpts test.DependancyOption) {
		...
		defer svc.Stop(ctx)
		tab := mustCreatePublishedTable(ctx, svc, t)
