...
func mustCreateMembershipTable(ctx context.Context, svc *frame.Service, t *testing.T) (tab tables.Membership, stateKeyTab tables.EventStateKeys) {
	t.Helper()

	cm := migrateDatabase(ctx, svc, t)

	tab = postgres.NewPostgresMembershipTable(cm)

	stateKeyTab = postgres.NewPostgresEventStateKeysTable(cm)
...
func TestMembershipTable(t *testing.T) {
	test.WithAllDatabases(t, func(t *testing.T, testOpts test.DependancyOption) {
		...
		tab, stateKeyTab := mustCreateMembershipTable(ctx, svc, t)
