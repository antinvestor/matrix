...
func mustCreateStateBlockTable(ctx context.Context, svc *frame.Service, t *testing.T) tables.StateBlock {
	t.Helper()

	cm := migrateDatabase(ctx, svc, t)

	tab := postgres.NewPostgresStateBlockTable(cm)
	return tab
}
...
func TestStateBlockTable(t *testing.T) {
	test.WithAllDatabases(t, func(t *testing.T, testOpts test.DependancyOption) {
		...
		defer svc.Stop(ctx)
		tab := mustCreateStateBlockTable(ctx, svc, t)
